...
/* package */ final class CuePainter {
    ...
    public CuePainter(Context context) {
        ...
        int twoDpInPx = Math.round((2f * displayMetrics.densityDpi) / DisplayMetrics
                .DENSITY_DEFAULT);
     ...
     */
    public void draw(Cue cue, boolean applyEmbeddedStyles, CaptionStyleCompat style, float
            textSizePx,
                     float bottomPaddingFraction, Canvas canvas, int cueBoxLeft, int cueBoxTop,
                     int cueBoxRight,
                     int cueBoxBottom) {
        ...
        if (areCharSequencesEqual(this.cueText, cueText)
                && areEqual(this.cueTextAlignment, cue.textAlignment)
                && this.cueLine == cue.line
                && this.cueLineType == cue.lineType
                && areEqual(this.cueLineAnchor, cue.lineAnchor)
                && this.cuePosition == cue.position
                && areEqual(this.cuePositionAnchor, cue.positionAnchor)
                ...
                && areEqual(this.textPaint.getTypeface(), style.typeface)
        ...
        Layout.Alignment textAlignment = cueTextAlignment == null ? Layout.Alignment.
                ALIGN_CENTER : cueTextAlignment;
        textLayout = new StaticLayout(cueText, textPaint, availableWidth, textAlignment,
                                      spacingMult,
                                      spacingAdd, true);
        ...
        if (cuePosition != Cue.DIMEN_UNSET) {
            ...
            textLeft = cuePositionAnchor == Cue.ANCHOR_TYPE_END ? anchorPosition - textWidth
                    : cuePositionAnchor == Cue.ANCHOR_TYPE_MIDDLE ? (anchorPosition * 2 -
                    textWidth) / 2
                    : anchorPosition;
        ...
        if (cueLine != Cue.DIMEN_UNSET) {
            ...
            textTop = cueLineAnchor == Cue.ANCHOR_TYPE_END ? anchorPosition - textHeight
                    : cueLineAnchor == Cue.ANCHOR_TYPE_MIDDLE ? (anchorPosition * 2 - textHeight)
                    / 2
                    : anchorPosition;
        ...
        this.textLayout = new StaticLayout(cueText, textPaint, textWidth, textAlignment,
                                           spacingMult,
                                           spacingAdd, true);
    ...
    /**
     * This method is used instead of {@link TextUtils#equals(CharSequence, CharSequence)} because
     * the latter only checks the text of each sequence, and does not check for equality of styling
     * that may be embedded within the {@link CharSequence}s.
     */
    private static boolean areCharSequencesEqual(CharSequence first, CharSequence second) {
        // Some CharSequence implementations don't perform a cheap referential equality check in
        // their equals methods, so we perform one explicitly here.
