...
class TTFAnalyzer {
     ...
     *
     * @param fontFilename File absolute name.
     * @return Font name embedded in the file.
     */
    public String getTtfFontName(String fontFilename) {
        ...
        try {
            ...
            // The TTF file consist of several sections called "tables", and we need to know how
            ...
            for (int i = 0; i < numTables; i++) {
                ...
                if (tag == 0x6E616D65) {
                    ...
                    // This is also a table. See http://developer.apple
                    // .com/fonts/ttrefman/rm06/Chap6name.html
                    // According to Table 36, the total number of table records is stored in the
                    // second word, at the offset 2.
                    ...
                    for (int record = 0; record < count; record++) {
                        // Table 37 tells us that each record is 6 words -> 12 bytes, and that
                        // the nameID is 4th word so its offset is 6.
                        // We also need to account for the first 6 bytes of the header above
                        // (Table 36), so...
                        int nameid_offset = record * 12 + 6;
                        int platformID = getWord(table, nameid_offset);
                        int nameid_value = getWord(table, nameid_offset + 6);

                        // Table 42 lists the valid name Identifiers. We're interested in 4 but
                        // not in Unicode encoding (for simplicity).
                        // The encoding is stored as PlatformID and we're interested in Mac encoding
                        if (nameid_value == 4 && platformID == 1) {
                            // We need the string offset and length, which are the word 6 and 5
                            // respectively
